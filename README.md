...
## Music
This project uses [Miniaudio](https://github.com/dr-soft/miniaudio) for music playback if the original game CD is not available. For standard Quake 2, copy all tracks to `baseq2/music` folder following the `trackXX.ogg/flac/mp3/wav` naming scheme (where `XX` corresponds to two digit track number). For "Ground Zero" and "The Reckoning" copy the files to `rogue/music` and `xatrix/music` folders respectively.
